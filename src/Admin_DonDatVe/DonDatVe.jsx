...
import { Ticket, CheckCircle2, Hourglass, XCircle, TrendingUp, Search, Check, X, Eye, MapPin, Calendar, Clock, Popcorn } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';

export default function DonDatVe() {
    ...
    fetch('http://localhost:8080/cineaura-backend/api/bookings.php')
      ...
      .then(data => {
        const formattedData = data.map(item => ({
          ...
          trangThai: item.status, // PAID, PENDING, CANCELLED, USED
          phimTen: item.movie_title || 'Phim đã bị gỡ',
          // Bổ sung dữ liệu chi tiết cho E-Ticket (nếu API có trả về, nếu không dùng dữ liệu giả lập để thiết kế form)
  ...
  const setStatus = (db_id, newStatus) => {
    const formData = new FormData();
    formData.append('id', db_id);
    formData.append('status', newStatus);

    fetch('http://localhost:8080/cineaura-backend/api/update_booking_status.php', {
      method: 'POST',
      body: formData
    })
    .then(res => res.json())
    .then(data => {
      if(data.success) loadDonHang();
  ...
  return (
    <div className="space-y-6 pb-12 font-sans text-gray-200 text-left">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-brand-dark-card border border-white/5 p-6 rounded-3xl relative overflow-hidden">
        ...
        <div className="space-y-1 relative z-10">
          <div className="flex items-center space-x-1.5 text-brand-amber font-mono text-[9px] uppercase tracking-widest font-bold">
            <Ticket className="w-3.5 h-3.5 text-brand-amber" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="p-5 bg-brand-dark-card border border-white/5 rounded-2xl flex items-center justify-between">
      ...
      <div className="bg-brand-dark-card border border-white/5 rounded-3xl p-6 sm:p-8 space-y-6">
        <div className="relative">
          <input
            ...
            className="w-full bg-black/50 border border-white/5 py-3 px-4 pl-10 rounded-xl text-sm text-white focus:outline-none focus:border-brand-amber"
          />
            ...
            <tbody className="divide-y divide-white/5 bg-black/15">
              {filteredList.map((item) => (
                <tr key={item.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-5 py-4 font-mono font-bold text-center text-brand-amber">{item.id}</td>
                  <td className="px-5 py-4"><strong>{item.khachHang}</strong><p className="text-[10px] text-gray-500">{item.email}</p></td>
                  <td className="px-5 py-4"><strong>{item.phimTen}</strong><p className="text-[10px] text-brand-amber font-mono mt-0.5">Ghế: {item.gheDat}</p></td>
                  ...
                  <td className="px-5 py-4 text-center">
                    <div className="flex justify-center items-center space-x-1.5">
                      {/* Nút Xem Chi Tiết Vé */}
                      <button onClick={() => setSelectedTicket(item)} className="p-1.5 bg-blue-500/10 text-blue-400 rounded hover:bg-blue-500/20 cursor-pointer border border-blue-500/20 transition-all" title="Xem chi tiết vé">
                        <Eye className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => setStatus(item.db_id, 'PAID')} className="p-1.5 bg-emerald-500/10 text-emerald-400 rounded hover:bg-emerald-500/20 cursor-pointer border-0" title="Đánh dấu đã thanh toán"><Check className="w-3.5 h-3.5" /></button>
                      <button onClick={() => setStatus(item.db_id, 'CANCELLED')} className="p-1.5 bg-red-500/10 text-red-400 rounded hover:bg-red-500/20 cursor-pointer border-0" title="Hủy đơn"><X className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <AnimatePresence>
        {selectedTicket && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {/* Thân vé điện tử */}
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl z-20 overflow-hidden font-sans text-slate-800">
              ...
              <div className="relative h-48 w-full bg-slate-900">
                <img src={selectedTicket.poster} alt="Poster" className="w-full h-full object-cover opacity-60" />
                <div className="absolute inset-0 bg-gradient-to-t from-white via-white/20 to-transparent" />
                <div className="absolute bottom-4 left-6 right-6">
                  <span className="text-[10px] bg-[#dfa112] text-black font-bold uppercase tracking-widest px-2.5 py-1 rounded shadow-md font-mono mb-2 inline-block">CineAura Ticket</span>
              ...
              <div className="px-6 py-5 space-y-4 relative bg-white">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex justify-between items-center mt-2">
                  <div>
                    ...
                    <p className="text-xl font-black text-[#dfa112] font-mono">{selectedTicket.gheDat}</p>
                  </div>
              ...
              <div className="relative h-6 bg-white overflow-hidden flex items-center">
                <div className="absolute left-[-12px] w-6 h-6 bg-black/80 rounded-full shadow-inner" />
                <div className="w-full border-t-[2.5px] border-dashed border-slate-200" />
                <div className="absolute right-[-12px] w-6 h-6 bg-black/80 rounded-full shadow-inner" />
              </div>
